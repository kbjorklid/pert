...
export class MonteCarloAlgorithm implements EstimationAlgorithm {
    name = 'Monte Carlo';

    calculate(storiesEstimates: Estimate[][], iterations: number = 50000): SimulationResult {
        ...
        for (let i = 0; i < iterations; i++) {
            for (const param of storyParams) {
                if ('fixed' in param) {
                    ...
                    const val = this.sampleBeta(param.alpha, param.betaVal);
        ...
        const percentiles = {
            ...
            p70: this.calculatePercentile(sortedSamples, 70),
            p80: this.calculatePercentile(sortedSamples, 80),
    ...
    private calculatePercentile(sortedData: Float64Array, percentile: number): number {
